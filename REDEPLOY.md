...
- **Первый раз:** выполни шаги 1–2 (переменные и домен), затем 4 (пуш кода). Бот запускаешь локально (шаг 5).
...
| `BOT_ON_START_SECRET` | Секретная строка для вызова `/api/bot/on-start`. Придумай длинную случайную строку (например сгенерируй: `openssl rand -hex 32`) |
...
**Важно:** `BOT_ON_START_SECRET` на Railway и в твоём локальном `.env` (для бота) должны совпадать — бот при `/start` вызывает бэкенд с этим секретом.
...
Бот должен работать у тебя на компьютере (или на любом сервере с доступом в интернет). Он при каждом `/start` вызывает бэкенд на Railway.
...
Оставь терминал открытым. В логе должно появиться что-то вроде: `Бот запущен: t.me/YourBot`.
